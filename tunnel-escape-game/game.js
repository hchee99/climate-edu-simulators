...
(function () {
  ...
  // ---- 맵/게임 상수 ----
  const COLS = 5;            // 길 개수
  const WROWS = 18;          // 월드 전체 칸 수 (맵 길이)
  const VIS = 9;             // 화면에 보이는 칸 수
  const CW = cv.width / COLS;
  const CH = cv.height / VIS;
  const WORLDH = WROWS * CH;
  const RAILS = [0, 4];      // 손잡이(안전) 길
  const TOTAL = 45;          // 골든타임(초)

  let player, items, timeLeft, score, running, tubeUntil, pushAcc, last, msgFlash, camY;

  // ---- 게임 초기화 ----
  function reset() {
    player = { col: 2, row: WROWS - 1 };
    items = [];
    const types = ["time", "ladder", "tube", "time", "ladder", "tube", "time"];
    let placed = 0, tries = 0;
    while (placed < 7 && tries < 120) {
      tries++;
  ...
  const waterRows = () => (1 - timeLeft / TOTAL) * WROWS;       // 바닥부터 잠긴 칸 수
  ...
  function move(dc, dr) {
    ...
    player.row = Math.max(0, Math.min(WROWS - 1, player.row + dr));
    collect(); updateCam();
    if (player.row === 0) win();
  }
  ...
  function collect() {
    items.forEach((it) => {
      ...
      if (it.type === "time") { timeLeft += 5; flash("+5초!"); }
      if (it.type === "ladder") {
